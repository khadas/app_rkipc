...
extern rtsp_session_handle g_rtsp_session_0, g_rtsp_session_1;
	...
	while (g_audio_run_) {
		...
		if (s32ret == RK_SUCCESS) {
			...
			if (buffer) {
				...
				if (g_rtsplive) {
					if (g_rtsp_session_0)
						rtsp_tx_audio(g_rtsp_session_0, buffer, pstStream.u32Len, pstStream.u64TimeStamp);
					if (g_rtsp_session_1)
						rtsp_tx_audio(g_rtsp_session_1, buffer, pstStream.u32Len, pstStream.u64TimeStamp);
